...
/* Note: this header includes #defines to adjust the names of the global
 * functions in this file to be in the mmint_ namespace to avoid namespace
 * conflicts. */
...
static int entropy_poll(void *user_arg, uint8_t *output, size_t len,
                        size_t *out_len) {
    uint32_t r;
...
int crypto_get_random(void *buf, size_t len) {
    return mbedtls_ctr_drbg_random(crypto_mbedtls_ctr_drbg(), buf, len) ? -1
                                                                        : 0;
}
...
static int hmac_prf_bits(const uint8_t *key, size_t key_len, const char *label,
    ...
    const uint8_t *const addr[] = {(uint8_t *)&ctr, (uint8_t *)label, data,
                                   (uint8_t *)&n_le};
...
int sha512_prf(const uint8_t *key, size_t key_len, const char *label,
               const uint8_t *data, size_t data_len, uint8_t *buf,
               size_t buf_len) {
    return hmac_prf_bits(key, key_len, label, data, data_len, buf, buf_len * 8,
                         MBEDTLS_MD_SHA512);
}
...
int sha256_prf_bits(const uint8_t *key, size_t key_len, const char *label,
                    const uint8_t *data, size_t data_len, uint8_t *buf,
                    size_t buf_len_bits) {
    return hmac_prf_bits(key, key_len, label, data, data_len, buf, buf_len_bits,
                         MBEDTLS_MD_SHA256);
}
...
int pbkdf2_sha1(const char *passphrase, const uint8_t *ssid, size_t ssid_len,
    ...
    return mbedtls_pkcs5_pbkdf2_hmac_ext(
               MBEDTLS_MD_SHA1, (const uint8_t *)passphrase, strlen(passphrase),
               ssid, ssid_len, iterations, 32, buf)
               ? -1
               : 0;
#else
    ...
    int ret = mbedtls_md_setup(&ctx, md_info, 1) ||
                      mbedtls_pkcs5_pbkdf2_hmac(
                          &ctx, (const uint8_t *)passphrase, strlen(passphrase),
                          ssid, ssid_len, iterations, 32, buf)
                  ? -1
                  : 0;
    mbedtls_md_free(&ctx);
...
int crypto_bignum_to_bin(const struct crypto_bignum *a, uint8_t *buf,
                         size_t buflen, size_t padlen) {
...
int crypto_bignum_add(const struct crypto_bignum *a,
    ...
    return mbedtls_mpi_add_mpi((mbedtls_mpi *)c, (const mbedtls_mpi *)a,
                               (const mbedtls_mpi *)b)
               ? -1
               : 0;
}
...
int crypto_bignum_mod(const struct crypto_bignum *a,
                      const struct crypto_bignum *b, struct crypto_bignum *c) {
    return mbedtls_mpi_mod_mpi((mbedtls_mpi *)c, (const mbedtls_mpi *)a,
                               (const mbedtls_mpi *)b)
               ? -1
               : 0;
}
...
int crypto_bignum_exptmod(const struct crypto_bignum *a,
    ...
    if (b == d || c == d) /*(not ok; store result in intermediate)*/
    {
        mbedtls_mpi R;
        mbedtls_mpi_init(&R);
        int rc = mbedtls_mpi_exp_mod(&R, (const mbedtls_mpi *)a,
                                     (const mbedtls_mpi *)b,
...
int crypto_bignum_sub(const struct crypto_bignum *a,
                      const struct crypto_bignum *b, struct crypto_bignum *c) {
    return mbedtls_mpi_sub_mpi((mbedtls_mpi *)c, (const mbedtls_mpi *)a,
                               (const mbedtls_mpi *)b)
               ? -1
               : 0;
}
...
int crypto_bignum_div(const struct crypto_bignum *a,
    ...
    mbedtls_mpi R;
    mbedtls_mpi_init(&R);
    int rc = mbedtls_mpi_div_mpi(&R, NULL, (const mbedtls_mpi *)a,
                                 (const mbedtls_mpi *)b) ||
...
int crypto_bignum_addmod(const struct crypto_bignum *a,
                         ...
                         const struct crypto_bignum *c,
                         struct crypto_bignum *d) {
    return mbedtls_mpi_add_mpi((mbedtls_mpi *)d, (const mbedtls_mpi *)a,
                               (const mbedtls_mpi *)b) ||
...
int crypto_bignum_mulmod(const struct crypto_bignum *a,
                         ...
                         const struct crypto_bignum *c,
                         struct crypto_bignum *d) {
    return mbedtls_mpi_mul_mpi((mbedtls_mpi *)d, (const mbedtls_mpi *)a,
                               (const mbedtls_mpi *)b) ||
                   mbedtls_mpi_mod_mpi((mbedtls_mpi *)d, (mbedtls_mpi *)d,
                                       (const mbedtls_mpi *)c)
               ? -1
...
int crypto_bignum_legendre(const struct crypto_bignum *a,
    ...
    if (mbedtls_mpi_sub_int(&exp, (const mbedtls_mpi *)p, 1) == 0 &&
        ...
        unsigned int mask;
        res = -1;
        mask = const_time_eq(crypto_bignum_is_one((struct crypto_bignum *)&tmp),
                             1);
...
struct crypto_ecdh *crypto_ecdh_init(int group) {
    ...
    struct crypto_ecdh *ecdh =
        crypto_mbedtls_keypair_gen(group, &pk) == 0
...
struct wpabuf *crypto_ecdh_get_pubkey(struct crypto_ecdh *ecdh, int inc_y) {
    ...
    if (mbedtls_ecp_point_write_binary(grp, &ecdh->Q, inc_y, &len, buf,
                                       sizeof(buf)) == 0) {
...
static int crypto_mbedtls_short_weierstrass_derive_y(mbedtls_ecp_group *grp,
    ...
    mbedtls_mpi *cy2 = (mbedtls_mpi *)crypto_ec_point_compute_y_sqr(
        (struct crypto_ec *)grp, (const struct crypto_bignum *)bn); /*x*/
    if (cy2 == NULL) {
        return -1;
    }
...
struct wpabuf *crypto_ecdh_set_peerkey(struct crypto_ecdh *ecdh, int inc_y,
    ...
    if (mbedtls_ecdh_calc_secret(&ecdh->ctx, &len, wpabuf_mhead(buf), len,
                                 mbedtls_ctr_drbg_random,
...
size_t crypto_ec_prime_len(struct crypto_ec *e) { return CRYPTO_EC_plen(e); }
...
int crypto_ec_point_to_bin(struct crypto_ec *e,
                           const struct crypto_ec_point *point, uint8_t *x,
                           uint8_t *y) {
    /* future: to avoid MBEDTLS_PRIVATE(), could write pt to stack buf,
...
struct crypto_ec_point *crypto_ec_point_from_bin(struct crypto_ec *e,
    ...
    if (mbedtls_ecp_get_type((mbedtls_ecp_group *)e) ==
        ...
        if (mbedtls_ecp_point_read_binary((mbedtls_ecp_group *)e, p, buf,
                                          1 + len * 2) == 0) {
            return (struct crypto_ec_point *)p;
        }
#endif
...
int crypto_ec_point_add(struct crypto_ec *e, const struct crypto_ec_point *a,
    ...
    int ret = mbedtls_mpi_lset(&one, 1) ||
                      mbedtls_ecp_muladd((mbedtls_ecp_group *)e,
                                         (mbedtls_ecp_point *)c, &one,
                                         (const mbedtls_ecp_point *)a, &one,
...
int crypto_ec_point_mul(struct crypto_ec *e, const struct crypto_ec_point *p,
    ...
    return mbedtls_ecp_mul((mbedtls_ecp_group *)e, (mbedtls_ecp_point *)res,
...
int crypto_ec_point_invert(struct crypto_ec *e, struct crypto_ec_point *p) {
    if (mbedtls_ecp_get_type((mbedtls_ecp_group *)e) ==
        ...
        wpa_printf(MSG_ERROR, "%s not implemented for Montgomery curves",
                   __func__);
        return -1;
    }
    ...
    return mbedtls_ecp_is_zero((mbedtls_ecp_point *)p) || /*point at infinity*/
                   mbedtls_mpi_cmp_int(py, 0) ==
                       0 || /*point is its own inverse*/
...
int crypto_ec_point_is_on_curve(struct crypto_ec *e,
    ...
    mbedtls_mpi *cy2 = (mbedtls_mpi *)crypto_ec_point_compute_y_sqr(
        e, (const struct crypto_bignum *)px);
    if (cy2 == NULL) {
        return 0;
    }
...
int aes_wrap(const uint8_t *kek, size_t kek_len, int n, const uint8_t *plain,
    ...
    int ret = mbedtls_nist_kw_setkey(&ctx, MBEDTLS_CIPHER_ID_AES, kek,
                                     kek_len * 8, 1);
    if (ret != 0) {
        goto cleanup;
    }

    ret = mbedtls_nist_kw_wrap(&ctx, MBEDTLS_KW_MODE_KW, plain, n * 8, cipher,
                               &out_len, 8 * (n + 1));
