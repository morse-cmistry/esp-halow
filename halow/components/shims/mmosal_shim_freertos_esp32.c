...
/** Magic number to put in @c mmosal_assert_info.magic to indicate that the
 * assertion info is valid. */
#define ASSERT_INFO_MAGIC (0xabcd1234)

/* Persistent assertion info. Linker script should put this into memory that is
 * not zeroed on boot. Be careful to update linker script if renaming. */
struct mmosal_preserved_failure_info preserved_failure_info
...
void mmosal_log_failure_info(const struct mmosal_failure_info *info) {
    ...
    record_num =
        FAST_MOD(preserved_failure_info.failure_count, MAX_FAILURE_RECORDS);
...
static void mmosal_dump_failure_info(void) {
    ...
    unsigned new_failure_count = preserved_failure_info.failure_count -
    ...
    for (failure_offset = 0; failure_offset < new_failure_count;
         failure_offset++) {
        unsigned ii;
        unsigned idx =
            FAST_MOD(first_failure_num + failure_offset, MAX_FAILURE_RECORDS);
        struct mmosal_failure_info *info = &preserved_failure_info.info[idx];

        ets_printf(
            "Failure %u logged at pc 0x%08lx, lr 0x%08lx, line %ld in %08lx\n",
            first_failure_num + failure_offset, info->pc, info->lr, info->line,
            info->fileid);

        for (ii = 0;
             ii < sizeof(info->platform_info) / sizeof(info->platform_info[0]);
             ii++) {
...
void *mmosal_malloc_(size_t size) { return pvPortMalloc(size); }
...
void mmosal_free(void *p) { vPortFree(p); }

void *mmosal_realloc(void *ptr, size_t size) { return realloc(ptr, size); }
...
struct mmosal_task *mmosal_task_create(mmosal_task_fn_t task_fn, void *argument,
    ...
    struct mmosal_task_arg *task_arg =
        (struct mmosal_task_arg *)mmosal_malloc(sizeof(*task_arg));
    if (task_arg == NULL) {
        return NULL;
    }
    ...
    BaseType_t result = xTaskCreate(mmosal_task_main, name, stack_size_u32 * 4,
                                    task_arg, freertos_priority, &handle);
    if (result == pdFAIL) {
...
/*
 * Warning: this function should not be used since eTaskGetState() is not a
 * reliable means of testing whether a task has completed.
 *
...
void mmosal_task_yield(void) { taskYIELD(); }
...
void mmosal_task_enter_critical(void) { taskENTER_CRITICAL(&task_spinlock); }

void mmosal_task_exit_critical(void) { taskEXIT_CRITICAL(&task_spinlock); }

void mmosal_disable_interrupts(void) { taskDISABLE_INTERRUPTS(); }

void mmosal_enable_interrupts(void) { taskENABLE_INTERRUPTS(); }
...
#if (configUSE_TRACE_FACILITY == 1) && defined(ENABLE_TRACEALYZER) &&          \
    ENABLE_TRACEALYZER
    if (name != NULL) {
...
bool mmosal_mutex_is_held_by_active_task(struct mmosal_mutex *mutex) {
    return xSemaphoreGetMutexHolder((SemaphoreHandle_t)mutex) ==
...
struct mmosal_sem *mmosal_sem_create(unsigned max_count, unsigned initial_count,
                                     const char *name) {
...
bool mmosal_semb_give_from_isr(struct mmosal_semb *semb) {
    ...
    BaseType_t ret =
        xSemaphoreGiveFromISR((SemaphoreHandle_t)semb, &task_woken);
    if (ret == pdPASS) {
        portYIELD_FROM_ISR(task_woken);
        return true;
    } else {
...
struct mmosal_queue *mmosal_queue_create(size_t num_items, size_t item_size,
                                         const char *name) {
    struct mmosal_queue *queue =
        (struct mmosal_queue *)xQueueCreate(num_items, item_size);
#if (configUSE_TRACE_FACILITY == 1) && defined(ENABLE_TRACEALYZER) &&          \
    ENABLE_TRACEALYZER
    if (name != NULL) {
...
bool mmosal_queue_pop(struct mmosal_queue *queue, void *item,
                      uint32_t timeout_ms) {
    uint32_t timeout_ticks = portMAX_DELAY;
    if (timeout_ms != UINT32_MAX) {
        timeout_ticks = timeout_ms / portTICK_PERIOD_MS;
    }
    return (xQueueReceive((SemaphoreHandle_t)queue, item, timeout_ticks) ==
            pdPASS);
}

bool mmosal_queue_push(struct mmosal_queue *queue, const void *item,
                       uint32_t timeout_ms) {
    uint32_t timeout_ticks = portMAX_DELAY;
    if (timeout_ms != UINT32_MAX) {
        timeout_ticks = timeout_ms / portTICK_PERIOD_MS;
    }
    return (xQueueSendToBack((SemaphoreHandle_t)queue, item, timeout_ticks) ==
            pdPASS);
}

bool mmosal_queue_pop_from_isr(struct mmosal_queue *queue, void *item) {
    BaseType_t task_woken = pdFALSE;
    if (xQueueReceiveFromISR((SemaphoreHandle_t)queue, item, &task_woken) ==
        pdTRUE) {
        portYIELD_FROM_ISR(task_woken);
        return true;
    } else {
...
bool mmosal_queue_push_from_isr(struct mmosal_queue *queue, const void *item) {
    BaseType_t task_woken = pdFALSE;
    if (xQueueSendToBackFromISR((SemaphoreHandle_t)queue, item, &task_woken) ==
        pdTRUE) {
        portYIELD_FROM_ISR(task_woken);
        return true;
    } else {
...
uint32_t mmosal_get_time_ticks(void) { return xTaskGetTickCount(); }

uint32_t mmosal_ticks_per_second(void) { return portTICK_PERIOD_MS * 1000; }
...
/**
 * @struct mmosal_timer
 * @brief Structure representing a timer in the MMOSAL (Morse Micro OS
 * Abstraction Layer)
 *
 * This structure encapsulates the necessary information for managing an ESP
 * timer.
 */
struct mmosal_timer {
...
bool mmosal_timer_start(struct mmosal_timer *timer) {
    ...
    if (timer->auto_reload) {
        return esp_timer_start_periodic(timer->handle, timer->period_us) ==
               ESP_OK;
    } else {
