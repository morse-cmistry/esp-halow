...
static void wlan_hal_gpio_init(void) {
    ...
    io_conf.pin_bit_mask =
        ((1ull << CONFIG_MM_WAKE) | (1ull << CONFIG_MM_SPI_CS));
...
static void wlan_hal_spi_init(void) {
    ...
    spi_bus_config_t buscfg = {
        ...
        .quadwp_io_num = -1,
        .quadhd_io_num = -1,
        /* max_transfer_sz defaults to 4092 if 0 when DMA enabled, or to
    ...
    /* Selected the highest available SPI clock speed that is still below the
     * MM6108's maximum of 50MHz */
    ...
    /* The actual clock frequency may not be the one that was set as it is
     * re-calculated by the driver to the nearest hardware-compatible number.
     * Importantly it is the "nearest", so it could be above the value set. */
...
/**
 * Minium transfer length in bytes before interrupt based transactions are used.
 * This is because there is some setup time associated with using the interrupt
 * based method when compared to the polling method. In the cases where the
 * difference in setup time exceeds the transaction duration it is more
 * efficient to uses the polling method instead of the interrupt based one. The
 * below equation was used to calculate this.
 *
...
void mmhal_wlan_spi_cs_assert(void) { gpio_set_level(CONFIG_MM_SPI_CS, 0); }

void mmhal_wlan_spi_cs_deassert(void) { gpio_set_level(CONFIG_MM_SPI_CS, 1); }
...
void mmhal_wlan_send_training_seq(void) {
     ...
     * This method of stacking up the TX data is described in RM0090 rev 19
     * Figure 253. It results is a reduction in the time between bytes of ~85%
     * (316ns -> 48ns). Could not get this to work for the other transactions
     * however.
     */
...
void mmhal_wlan_deinit(void) {
    /* Lower the RESET_N line to disable the WLAN transceiver. This will put the
     * transceiver in its lowest power state. */
    gpio_set_level(CONFIG_MM_RESET_N, 0);

    wlan_hal_spi_deinit();

    /* Clean up any ISR handlers that have been added. These will be added again
     * if the WLAN interface is brought back up. */
    gpio_isr_handler_remove(CONFIG_MM_SPI_IRQ);
    gpio_isr_handler_remove(CONFIG_MM_BUSY);
}

void mmhal_wlan_wake_assert(void) { gpio_set_level(CONFIG_MM_WAKE, 1); }

void mmhal_wlan_wake_deassert(void) { gpio_set_level(CONFIG_MM_WAKE, 0); }
